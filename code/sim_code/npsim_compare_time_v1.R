here::i_am("code/sim_code/npsim_compare_time_v1.R")
...
# ) %dopar% {
  ...
  n.bench = bench::mark(get_NOISeq(countdat = countdat, design_mat = design_mat),memory = F,filter_gc=F,iterations = itermax,time_unit = "s"),
  x.bench = bench::mark(get_dearseq(countdat = countdat, design_mat = design_mat),memory = F,filter_gc=F,iterations = itermax,time_unit = "s")
  )


saveRDS(object = bench.list, file = here("output", "bench_time", paste0(sim.name, ".btime.new.RDS")))
