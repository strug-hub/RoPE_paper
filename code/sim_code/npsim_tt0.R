here::i_am("code/sim_code/npsim_tt0.R")
...
retmat <- foreach (iterindex = seq_len(itermax),
                   .combine = rbind) %dopar% {
                     ...
                     ## Return ----------------------------------------------------------
                     retvec <- c(fprvec, powervec, msevec, mpve = mpve)

                     retvec
                   }
stopCluster(cl)

saveRDS(object = retmat, file = here("output","diff_exp_out",paste0(sim.name,".NP.RDS")))
