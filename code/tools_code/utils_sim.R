...
.bulk.NB.RNAseq_counts <- function(sim.options, phenotype, modelmatrix, coef.dat) {
  ...
  if (attr(sim.options, 'param.type') == 'estimated') {
    ...
    lmu = log2(true.means + 1)
    predsize.mean = stats::approx(meansizefit$x, meansizefit$y, xout = lmu, ties = "ordered")$y
    predsize.sd = stats::approx(meansizefit$x, meansizefit$sd, xout = lmu, ties = "ordered")$y
